...
export default function FileUploadDialog({ 
  ...
  const handleCaseSelect = (caseId: string) => {
    ...
    // 載入該案件的可用資料夾
    const folders = FolderManager.getAvailableFolders(caseId);
                ...
                {selectedCaseIds.map(caseId => {
                  ...
                  return (
                    <option key={caseId} value={caseId}>
                      {caseData ? `${caseData.client} - ${caseData.caseNumber}` : caseId}
                    </option>
                  );
                })}
              </select>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {availableFolders.map(folder => (
                  <label key={folder.path} className="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50">
                    <input
                      type="radio"
                      name="folder"
                      value={folder.path}
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-24 overflow-y-auto">
                    {selectedFiles.map((file, index) => (
                      <div key={index} className="flex items-center justify-between text-xs text-gray-600 bg-gray-50 rounded px-2 py-1 mb-1">
